...
class Auth {
	...
	public static function authenticate($email, $senha){
		...
		$sql = 'SELECT u.*, g.nome as nome_grupo, GROUP_CONCAT(p.permissao SEPARATOR \',\') as permissoes FROM usuarios u INNER JOIN grupos g on u.grupoid = g.id INNER JOIN grupos_permissoes gp on gp.grupoid = g.id LEFT JOIN permissoes p on gp.permissaoid = p.id WHERE u.email = :email and u.senha = :senha and u.status = 1 GROUP BY u.id';
		$stmt = $db->prepare($sql);
		...
		if($usuario !== null){
			...
			Session::put('idtipo_usuario', $usuario->grupoid);
